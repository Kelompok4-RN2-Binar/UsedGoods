import { StyleSheet, Text, View ,SafeAreaView,StatusBar,ScrollView,Dimensions,Image,NativeModules,Linking,RefreshControl,TouchableOpacity} from 'react-native'
...
const InfoPenawar = ({route}) => {
    ...
    console.log("data penawar r: ",data)
            ...
            <View style={{justifyContent:'center',alignItems:'center',width:window.width*0.9,alignSelf:'center'}}>
                ...
                <View style={{flexDirection: 'row',paddingTop:20,width:window.width*0.9,marginLeft:20}}>
                  ...
                  <View style={{flexDirection: 'column'}}>
                    <Text style={[styles.Text,{fontSize:14}]}>{data.User.full_name}</Text>
                    <Text style={[styles.Text,{fontSize:14}]}>{data.User.city}</Text>
                  </View>
                </View>
                <View style={{flexDirection: 'row',paddingTop:20,width:window.width*0.9,marginLeft:20}}>
                  <Image
    ...
    const onOpenStatus = (status) => {
        ...
        setComponent(
            <View style={{width:window.width}}>
              <View style={{justifyContent:'center',width:window.width*0.9,alignSelf:'center'}}>
                  <Text style={[styles.Text,{alignSelf:'center',fontSize:16,paddingTop:10,fontFamily:FONTS.SemiBold}]}>Update your product status</Text>
                  <TouchableOpacity style={{flexDirection: 'row',paddingTop:20,width:window.width*0.8,marginLeft:20}} onPress={()=>{setopenModal(false);;onOpenStatus('Sold')}}>
                    <View style={{borderRadius:15,width:20,height:20,backgroundColor:status=='Sold'?COLORS.green:'#d0d0d0'}}/>
                    <View style={{flexDirection:'column',marginLeft:10}}>
                      <Text style={[styles.Text,{fontSize:16}]}>Sold Out</Text>
                      <View style={{flexWrap:'wrap',flexDirection:'row'}}>
                        <Text style={[styles.Text,{color:COLORS.grey}]}>You have agreed to sell this product to the buyer</Text>
                      </View>
                    </View>
                  </TouchableOpacity>
                  <TouchableOpacity style={{flexDirection: 'row',paddingTop:20,width:window.width*0.8,marginLeft:20,marginBottom:20}} onPress={()=>{setopenModal(false);onDismiss();onOpenStatus('Cancel')}}>
                    <View style={{borderRadius:15,width:20,height:20,backgroundColor:status=='Cancel'?COLORS.red:'#d0d0d0'}}/>
                    <View style={{flexDirection:'column',marginLeft:10}}>
                      <Text style={[styles.Text,{fontSize:16}]}>Cancel Transaction</Text>
                      <View style={{flexWrap:'wrap',flexDirection:'row'}}>
                        <Text style={[styles.Text,{color:COLORS.grey}]}>You cancel this product transaction with the buyer</Text>
                      </View>
                    </View>
                  </TouchableOpacity>
    ...
    const onRefresh = useCallback(() => {
      dispatch(getWishlistSpesific(loginUser.access_token,dataRoute.id)).then(()=>{setRefreshing(true);})
      wait(500).then(() => {
        setRefreshing(false);
      });
    }, []);
    ...
    <SafeAreaView style={styles.Container}> 
      ...
      <ScrollView
        ...
        <View style={{margin:25,flexDirection:'column'}}>
            <Text style={[styles.Text,{fontSize:20}]}>List of products bid</Text>
            <View 
                key={data}
                style={{flexDirection: 'row',marginTop: 24,}}>
                <View >
                  <Image style={styles.image} source={{uri:data.Product.image_url}} />
                </View>
            ...
            {data.status=="pending" &&
            <View style={{flexDirection:'row'}}>
                <Button caption={'Decline'} onPress={()=>{dispatch(declineOrder(loginUser.access_token,data.id))}} style={{width:window.width*0.4,height:50,backgroundColor:COLORS.white,borderColor:COLORS.red,borderWidth:1}} styleText={{color:COLORS.black,fontFamily:FONTS.Regular}}/>
                <Button caption={'Accept'} style={{width:window.width*0.4,height:50}} 
                  onPress={()=>{
                    dispatch(acceptOrder(loginUser.access_token,data.id)).then(()=>{onOpenAccepted();})
                  }} />
            </View>
            }
